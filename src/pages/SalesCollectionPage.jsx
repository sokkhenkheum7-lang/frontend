...
import { categories, saleProducts, extraSaleProducts } from "../Data/salesData";

export default function SalesCollectionPage({ activeTab }) {
  const [selectedProduct, setSelectedProduct] = useState(null);
  const [activeCategory, setActiveCategory] = useState("Women's Sale");
  const [selectedColors, setSelectedColors] = useState({});
  const [wishlist, setWishlist] = useState({});

  // Reset product detail if user clicks the navigation tab again
  useEffect(() => {
  ...
  return (
    <main className="w-full bg-white relative select-none">
      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-8 md:px-12 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-2.5 gap-y-8">
